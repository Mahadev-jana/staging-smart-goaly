...
const FavoriteClubModal = React.lazy(() => import('./FavoriteClubModal'));
const DeleteFavoriteClubModal = React.lazy(() => import('./DeleteFavouriteClubsModal'));
                    ...
                    <div className="my-team bg-white border radius-1 row">
                        ...
                        <ul>
                            {clubs.slice(0, 3).map((club, key) => (
                                <li key={key} onClick={() => history.push(`/club-info/${club.id}`)}><LazyLoadImage src={club.badge} alt=""/></li>
                            ))}
                        </ul>
                        <a onClick={showAllClubs}><LazyLoadImage src={addButton} alt=""/></a>
                    </div>
                </div>}
            {expandClubs && <div className="block">
                <div className="my-team bg-white border radius-1 row" style={{ display: 'block' }}>
                    <div>
                        ...
                        <a onClick={() => setOpenAddModal(true)}>
                            {/* <img style={{ height: '50px', margin: '9px' }} src={imgAddClub} alt="" /> */}
                            <LazyLoadImage src={imgAddClub} alt=""style={{ height: '50px', margin: '9px' }}/>
                        </a>
                        <a onClick={() => setDeleteModal(true)}>
                            {/* <img style={{ height: '50px', margin: '9px' }} src={imgDeleteClub} alt="" /> */}
                            <LazyLoadImage src={imgDeleteClub} alt="" style={{ height: '50px', margin: '9px' }}/>
                        </a>
                        <a onClick={showThreeClubs}>
                            {/* <img style={{ height: '50px', margin: '9px' }} src={minusButton} alt="" /> */}
                            <LazyLoadImage src={minusButton} alt="" style={{ height: '50px', margin: '9px' }}/>
                            </a>
                        ...
                        {clubs.map((club, key) => (
                            <Col key={key} xs={3} onClick={() => history.push(`/club-info/${club.id}`)}>
                                <div className="pd-5">
                                    ...
                                    <LazyLoadImage src={club.badge} alt="" responsive style={{ height: 50, width: 50 }}/>
                                </div>
                                <div className="clearfix" style={{ clear: 'both' }}></div>
                            </Col>
                        ))}
    ...
    return (
        <div class="block">
            <div class="my-team bg-white border radius-1 row">
                ...
                <a onClick={onClickLogin}><LazyLoadImage src={addButton} alt=""/></a>
            </div>
        </div>
        // <Row>
